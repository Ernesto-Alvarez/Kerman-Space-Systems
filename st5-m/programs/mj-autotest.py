...
class vessel_monitor:
	...
	def monitor(self):
		...
		while self.state == "OK":			
			...
			if self.ap_time() > self.pe_time() and ascent.status not in ["Circularizing","Holding AP", "Coasting to edge of atmosphere","Coasting to circularization burn"]:
			...
			if self.angle_of_attack() > 20 and self.flight_info.mean_altitude < 35000:
...
class mj_autopilot:
	...
	def configure_mj(self):
		...
		st_controller.autostage_limit = 0
...
for j in ["R3-1200B-S2-N01N0R-ST5M-CLAW"]:
		print j
