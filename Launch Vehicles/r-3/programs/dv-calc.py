...
class data_recorder:
	...
	def record(self,mass,angle,result,dv):
		data[(mass,angle)] = (result,dv)

	def retrieve(self,mass,angle):
		try:
			return data[(mass,angle)]
		except ValueError:
			return ("N/A",0)

	def print(self):
		print "xx;25;50;75;100;125;150;175;200;225;250;275;300;325;350;375;400;425;450;475;500;525;550;575;600;625;650;675;700;725;750;775;800"
		for i in range(5,45):
			line = str(i)
			for j in range(25,801,25):
				line += ";" + retrieve
...
class vessel_monitor:
	...
	def propellant_mass(self):
		fuel_kg = self.lf() * self.vessel.resources.density('LiquidFuel')
		oxi_kg = self.ox() * self.vessel.resources.density('Oxidizer')
...
def launch(connection,template,mass,angle,turn_speed):

	fuel_units = (mass - 25) / 5

	with open(template, 'r') as file:
		data = file.read().replace('PUTXFUELXHERE', str(fuel_units) )

	savefile = '/home/ealvarez/GOG Games/Kerbal Space Program/game/saves/rocket tests/current.sfs'
	...
	ap = mj_autopilot(connection,vessel,turn_speed,i)
	time.sleep(10)
	ap.launch()
...
for j in ["/home/ealvarez/ksp/rocket tests/r-3/templates/R3-400-S1-H01N1X.sfs"]:
		print "pitch;deltav;time to orbit;ec remain;ap error;pe error;status"
		for i in [5,10]:
			for k in [100,200,300,400]:
				launch(connection,j,k,i,50)
